...
namespace UniLang
{
    public class UniManager : MonoBehaviour
    {
        ...
        public int newfontsize = -1;
        private int oldfontsize;
        // Start is called before the first frame update
        void Start()
        {
            thistext = this.GetComponent<Text>();
            oldfontsize = thistext.fontSize;
        ...
        void Update()
        {
            if (thistext.text != oldtext && !converttrg && GManager.instance.isEnglish != 0)
            {
                converttrg = true;
                oldtext = thistext.text;
                if (newfontsize != -1) thistext.fontSize = newfontsize;
                LangConvert();
            }
            else if (GManager.instance.isEnglish ==0 &&thistext.fontSize != oldfontsize) thistext.fontSize = oldfontsize;
        }
